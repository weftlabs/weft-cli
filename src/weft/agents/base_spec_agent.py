...
class BaseSpecAgent(BaseWatcher):
    ...
    def _resolve_spec_path(self) -> Path:
        ...
        if package_spec.exists():
            ...
            return package_spec  # type: ignore[no-any-return]
    ...
    def _load_conversation_history(self, conversation_id: str) -> list[dict]:
        ...
        pairs.sort(key=lambda x: x["timestamp"])  # type: ignore[arg-type,return-value]

        # Build messages array
        for pair in pairs:
            messages.append({"role": "user", "content": pair["prompt"]})  # type: ignore[dict-item]
            messages.append({"role": "assistant", "content": pair["result"]})  # type: ignore[dict-item]
